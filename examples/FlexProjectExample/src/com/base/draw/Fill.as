...
package com.base.draw {
	...
	public class Fill {
		...
		public static function image( element:Sprite, style:Object, t:* = null ) :void { 
			...
			if( !t.parent ){ bitmap = t.main.document.bitmaps[ s ]; }
			else { bitmap = t.parent.doc.bitmaps[ s ]; } 
			if( !bitmap ) { // load image if not already loaded
				...
				element.graphics.beginBitmapFill( bitmap );
				Shape[ style.shape.type ]( element, style );
			}
